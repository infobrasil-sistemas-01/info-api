...
describe('AccountReceivableService', () => {
  ...
  describe('get', () => {
    ...
    it('should return account receivables with arId filter', async () => {
      const result = await service.get('cred-1', 1, undefined, 10);

      expect(result).toBeDefined();
    });

    it('should return account receivables with situation filter', async () => {
      const result = await service.get('cred-1', 1, undefined, undefined, 'A');
    ...
    it('should throw BadRequestException when page is less than 1', async () => {
      await expect(service.get('cred-1', 0, 1)).rejects.toThrow(
        BadRequestException,
      );
      await expect(service.get('cred-1', 0, 1)).rejects.toThrow(
    ...
    it('should throw BadRequestException when page is greater than 100', async () => {
      await expect(service.get('cred-1', 101, 1)).rejects.toThrow(
        BadRequestException,
      );
      await expect(service.get('cred-1', 101, 1)).rejects.toThrow(
    ...
    it('should throw BadRequestException when no filter is provided', async () => {
      await expect(service.get('cred-1', 1)).rejects.toThrow(
        BadRequestException,
      );
      await expect(service.get('cred-1', 1)).rejects.toThrow(
    ...
    it('should throw BadRequestException for invalid situation value', async () => {
      await expect(
        service.get('cred-1', 1, undefined, undefined, 'X'),
    ...
    it('should accept situation "A" as valid', async () => {
      await expect(
        service.get('cred-1', 1, undefined, undefined, 'A'),
      ).resolves.toBeDefined();
    });

    it('should accept situation "L" as valid', async () => {
      await expect(
        service.get('cred-1', 1, undefined, undefined, 'L'),
      ).resolves.toBeDefined();
    });

    it('should throw BadRequestException for invalid startDate', async () => {
      await expect(
        service.get('cred-1', 1, undefined, undefined, undefined, 'invalid-date'),
      ).rejects.toThrow(BadRequestException);
      await expect(
        service.get('cred-1', 1, undefined, undefined, undefined, 'invalid-date'),
    ...
    it('should throw BadRequestException for invalid endDate', async () => {
      await expect(
        service.get('cred-1', 1, undefined, undefined, undefined, undefined, 'not-a-date'),
      ).rejects.toThrow(BadRequestException);
      await expect(
        service.get('cred-1', 1, undefined, undefined, undefined, undefined, 'not-a-date'),
    ...
    it('should throw BadRequestException when startDate is provided without endDate', async () => {
      await expect(
        service.get('cred-1', 1, undefined, undefined, undefined, '2024-01-01'),
      ).rejects.toThrow(BadRequestException);
      await expect(
        service.get('cred-1', 1, undefined, undefined, undefined, '2024-01-01'),
    ...
    it('should throw BadRequestException when endDate is provided without startDate', async () => {
      await expect(
        service.get('cred-1', 1, undefined, undefined, undefined, undefined, '2024-12-31'),
      ).rejects.toThrow(BadRequestException);
      await expect(
        service.get('cred-1', 1, undefined, undefined, undefined, undefined, '2024-12-31'),
    ...
    it('should throw error when query fails', async () => {
      ...
      await expect(service.get('cred-1', 1, 1)).rejects.toThrow('Query error');
    });
  });
});
