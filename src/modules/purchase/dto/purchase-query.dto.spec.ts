import { PurchaseQuerySchema } from './purchase-query.dto';

describe('PurchaseQueryDto Schema Validation', () => {
  const validData = {
    page: 1,
    pageSize: 10,
    storeId: 1,
  ...
  test.each([
    ...
    expect(() => PurchaseQuerySchema.parse(input)).not.toThrow();
  });
  ...
  test.each([
    ...
    expect(() => PurchaseQuerySchema.parse(input)).toThrow();
  });
});
